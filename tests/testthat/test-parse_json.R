...
simple_map_struct <- build_structure(s_map(
  name = s_string(),
  value = s_integer()
))
...
test_that("Parses simple atomic types correctly", {
  ...
  expect_equal(
    parse_json("\"hello world\"", build_structure(s_string())),
    "hello world"
  )
...
test_that("Parses nested structures correctly", {
  ...
  expect_equal(
    result$user[order(names(result$user))],
    expected$user[order(names(expected$user))]
  )
...
test_that("Parses vector of maps correctly", {
  ...
  for (i in seq_along(result)) {
    expect_equal(
      result[[i]][order(names(result[[i]]))],
      expected[[i]][order(names(expected[[i]]))]
    )
  }
...
test_that("Throws error on type mismatches", {
  ...
  expect_error(parse_json(
    "[1, \"2\", 3]",
    build_structure(s_vector(s_integer()))
  ))
  expect_error(parse_json(
    "[1, 2, true]",
    build_structure(s_vector(s_integer()))
  ...
  expect_error(parse_json(
    '{"id": 1, "user": {"username": "x", "active": "yes"}}',
    nested_struct
  ))
})
...
test_that("Throws error on missing required fields", {
  ...
  expect_error(parse_json(
    '{"id": 1, "user": {"username": "x"}, "tags": [], "scores": []}',
    nested_struct
  ))
})
...
test_that("Throws error on unknown/extra fields (default behavior)", {
  ...
  expect_error(parse_json(
    '{"name": "test", "value": 99, "extra": true}',
    simple_map_struct
  ))
  expect_error(parse_json(
    '{"id": 1, "user": {"username": "x", "active": true, "admin": false}, "tags": [], "scores": []}',
    nested_struct
  ))
})
...
test_that("Throws error on integer overflow for R's 32-bit integer", {
  ...
  expect_equal(
    parse_json(max_int_plus_1, build_structure(s_double())),
    2147483648
  )
})
...
test_that("Handles optional fields within maps correctly", {
  ...
  expect_equal(
    result1[order(names(result1))],
    expected1[order(names(expected1))]
  )
  ...
  expect_equal(
    result2[order(names(result2))],
    expected2[order(names(expected2))]
  )
...
test_that("Handles nested optional types", {
  ...
  expect_equal(
    parse_json('{"opt_val": 1.5}', s_nested_opt),
    list(opt_val = 1.5)
  )
  # Outer map is present, inner value is null
  expect_equal(
    parse_json('{"opt_val": null}', s_nested_opt),
    list(opt_val = NULL)
  )
  # Outer map is null
  ...
  s_vec_opt_map_opt_field <- build_structure(
    s_vector(
      s_optional(
        # Each element in the vector can be null OR a map
        s_map(
  ...
  for (i in seq_along(result_nested)) {
    if (is.list(result_nested[[i]])) {
      expect_equal(
        result_nested[[i]][order(names(result_nested[[i]]))],
  ...
  expect_error(parse_json(
    json_nested_err_req_key_miss,
    s_vec_opt_map_opt_field
  ))
